...
ARG=$(ruby -e "puts (0..($n)).to_a.shuffle.join(' ')"); $FILE $ARG | ./pro_checker $ARG
exit 0
fi
...
R=$(valgrind --log-fd=1 $FILE $ARG | grep -Ec 'no leaks are possible|ERROR SUMMARY: 0')
if [[ $R == 2 ]]; then
...
$FILE "--123 1 321" 2> test_check.txt
if [ -s "$FICHERO" ];then
while IFS= read -r line
do
  if [[ $line == "Error" ]]; then
...
$FILE "09 9 3 -1" 2> test_check.txt
if [ -s "$FICHERO" ];then
while IFS= read -r line
do
  if [[ $line == "Error" ]]; then
...
$FILE "00000001 1 9 3" 2> test_check.txt
if [ -s "$FICHERO" ];then
while IFS= read -r line
do
  if [[ $line == "Error" ]]; then
...
$FILE "-5 00000003 003 9 1" 2> test_check.txt
if [ -s "$FICHERO" ];then
while IFS= read -r line
do
  if [[ $line == "Error" ]]; then
...
R=$($FILE $ARG | valgrind --log-fd=1 ./checker $ARG | grep -Ec 'no leaks are possible|ERROR SUMMARY: 0')
if [[ $R == 2 ]]; then
  printf "${GREEN}[MOK] ${DEF_COLOR}";
else
  printf "${RED} [KO LEAKS] ${DEF_COLOR}";
