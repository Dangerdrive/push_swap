...
static void	move_a_to_b(t_stack_node **a, t_stack_node **b) //Define a function that prepares the cheapest nodes on top of the stacks for pushing `a` nodes to stack `b`, until there are three nodes left in `a`
{
	...
	if (cheapest_node->is_above_median 
		&& cheapest_node->target_node->is_above_median) //If both the cheapest `a` node and its target `b` node are above the median
		rotate_both(a, b, cheapest_node);
	else if (!(cheapest_node->is_above_median) 
		&& !(cheapest_node->target_node->is_above_median)) //If both the cheapest `a` node and its target `b` node are below the median
...
static void	rotate_min_to_top(t_stack_node **a) //Define a function that moves the smallest number to the top
{
	while ((*a)->value != get_min_value_node(*a)->value) //As long as the smallest number is not at the top
	{
		if (get_min_value_node(*a)->is_above_median) //Rotate or reverse rotate according to the position of the node on the median
			ra(a, false);
		else
			rra(a, false);
	}
...
void	sort_stacks(t_stack_node **a, t_stack_node **b) //Define a function that sorts stack `a` if it has more than 3 nodes
{
	...
	len_a = stack_len(*a);
	if (len_a-- > 3 && !is_stack_sorted(*a)) //If stack `a` has more than three nodes and aren't sorted
		pb(b, a, false);
	if (len_a-- > 3 && !is_stack_sorted(*a)) //If stack `a` still has more than three nodes and aren't sorted
		pb(b, a, false);
	while (len_a-- > 3 && !is_stack_sorted(*a)) //If stack `a` still has more than three nodes and aren't sorted
	{
